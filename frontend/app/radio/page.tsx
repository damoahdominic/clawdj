...
export default function Radio() {
  ...
  const handleVinylDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const audio = getActiveAudio();
    if (audio) {
      audio.playbackRate = 0.001; // near-zero instead of 0 (some browsers reject 0)
    }
  };
      ...
      <div className={`fixed top-0 right-0 h-full bg-gray-900/98 backdrop-blur-md border-l border-red-900/40 z-50 transition-transform duration-300 w-80 ${showSettings ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-6 space-y-6 h-full overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">How far into the track before crossfading to the next one</p>
            <input
              ...
              {playlist.map((track, i) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 text-left transition-all ${
                    i === currentIndex
                      ? "bg-gradient-to-r from-red-900/40 to-orange-900/20 border-l-3 border-orange-500"
                      : i < currentIndex
